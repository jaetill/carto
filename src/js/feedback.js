...
function injectButton() {
  if (document.getElementById('carto-feedback-btn')) return;

  const btn = document.createElement('button');
  btn.id = 'carto-feedback-btn';
  btn.type = 'button';
...
function openForm() {
  const existing = document.getElementById('carto-feedback-dialog');
  if (existing) { existing.focus(); return; }

  const dialog = document.createElement('dialog');
  dialog.id = 'carto-feedback-dialog';
  dialog.innerHTML = `
    <form method="dialog" id="carto-feedback-form" style="display:flex;flex-direction:column;gap:.75rem;min-width:320px;max-width:480px;padding:.5rem;">
  ...
  dialog.querySelector('#carto-feedback-cancel').addEventListener('click', () => {
    dialog.close();
    dialog.remove();
  });

  dialog.querySelector('#carto-feedback-form').addEventListener('submit', async (e) => {
    e.preventDefault();
